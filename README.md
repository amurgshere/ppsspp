...
- **[ALL] Kiosk Mode** — an alternate, gamepad-first UI for browsing and launching games, aimed at handheld/TV setups where you don't want the standard file-browser UI: a ROM carousel home screen (recent games + an "All ROMs" grid) in both portrait and landscape, a Game Info popup with save-state management (create/load/delete slots, delete game), and an on-screen button hint bar. Available on every platform, not just Switch. To try it: launch with the `--kiosk` command-line flag, or tick **Settings → System → Kiosk Mode → "Kiosk mode by default"** (`KioskModeDefault` in the ini) to always start into it.
...
- **[SWITCH] System shared-font text rendering** — Switch text (including CJK game titles) now renders via the console's system shared font instead of the old fixed bitmap atlas, fixing missing/tofu-boxed Japanese titles and descender clipping; also fixes a text overlap regression this introduced in the in-game Save/Load dialog.
...
- **[ALL] "Clear" button for savestate slots** — the pause screen's savestate slots now show a "Clear" button (after Save/Load) whenever that slot has a save, letting you delete an individual slot's data after confirming; supported in both the compact and full-height wide-button pause screen layouts.
- **[ALL] Auto save savestate** — new System setting automatically saves to a savestate slot when the game is closed from within PPSSPP (returning to the game list, or exiting PPSSPP entirely); not triggered by Reset, and not triggered by force-closing PPSSPP externally (e.g. via the Switch Home Menu's "Close" on the app icon), since that doesn't give PPSSPP a chance to run its normal shutdown handling. Slot selection mirrors "Auto Load Savestate": Off, First Empty Available then Oldest/Newest, Oldest or Slot 1, Newest or Slot 1, a specific slot, or "Last Loaded/Saved Save This Session, Next Empty, Oldest Save". Paired with an "Only auto save after seconds" slider to skip saving if too little progress has happened, and an "Always ask before auto saving" checkbox that, when on, prompts for confirmation (naming the slot that will be used/overwritten) before saving and before any exit-confirmation dialog.
- **[ALL] "Only auto load if selected savestate is newer than latest game save" setting** — on by default, this skips Auto Load Savestate if the game's own in-game save data is more recent than the savestate that would be loaded, so a newer in-game save is never silently clobbered by an older savestate.
- **[ALL] "Play without autoload" option** — new button on the game info screen's Play area, shown whenever Auto Load Savestate is enabled, to start the game without triggering the auto-load for that one launch.
